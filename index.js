...
class Calculator {
  ...
  clear() {
    if(this.currentOperand.innerText != '0' || this.previousOperand.innerText != '') {
      ...
      this.previousOperand.style.animation = 'hide-previous-value-animation 0.4s cubic-bezier(0.4, 1, 1, 1)';
      this.previousOperand.style.animationFillMode = 'forwards';
      setTimeout(() => {this.previousOperand.innerText = ""}, 200);
      //Set operator state.
  ...
  addNumber(element) {
    ...
    if(this.currentOperand.innerText.length < 8) this.currentOperand.innerText += element.innerText;
  }
  ...
  addOperator(element) {
    ...
    //add single operator.
    if(this.currentOperand.innerText != "0" && this.previousOperand.innerText == '' && !this.operationHasOperator) {
      //Calculate the operation.
      var operation = this.currentOperand.innerText + " " + valueOperator + " ";
      //Show the previousOperand animation.
      this.previousOperand.style.animation = 'show-previous-value-animation 0.4s cubic-bezier(0.4, 1, 1, 1)';
      this.previousOperand.style.animationFillMode = 'forwards';
      //Display the result.
  ...
  calculate(equalsButton = true) {
    if(this.currentOperand.innerText != '0' && this.previousOperand.innerText != '') {
      ...
      var operation = (this.previousOperand.innerText + this.currentOperand.innerText).replace(/[^-()\d/*+.]/g, '');;
      //Calculate the operation.
